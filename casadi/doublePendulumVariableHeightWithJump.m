...
desiredTimings = [0.6; 1.2; 0.5; 1.2; 0.6]; 
...
for k= 3 * phase_length + 1 : 4 * phase_length
  ...
  opti.subject_to(friction_function(X(:,k+1), U(1:2,k), xL2) <= staticFriction^2)
...
for k= 4 * phase_length + 1 : 5 * phase_length
  ...
  opti.subject_to(friction_function(X(:,k+1), U(4:5,k), xR2) <= staticFriction^2)
...
opti.minimize((T - desiredTimings)' * (T - desiredTimings) ...
    ...
    + 10/N * (sumsqr(U(1:2,:)) + sumsqr(U(4:5,:))) ...
